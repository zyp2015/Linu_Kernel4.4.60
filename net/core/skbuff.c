...
int sysctl_max_skb_frags   = MAX_SKB_FRAGS;
 ...
 */
struct sk_buff *skb_segment(struct sk_buff *head_skb,/*实现GSO报文分段的基础*/
			    netdev_features_t features)
{
	...
	headroom = skb_headroom(head_skb);/*得到报文的 headroom大小*/
	...
	do {
		...
		len = head_skb->len - offset;/*计算报文拷贝的长度 不包括包头*/
		if (len > mss)
			len = mss;
		...
		if (!hsize && i >= nfrags && skb_headlen(list_skb) &&/*frag_list中还有数据*/
		    (skb_headlen(list_skb) == len || sg)) {/*frag_list中的skb线性区长度不超过len，即mss值 */
		...
		skb_reset_mac_len(nskb);/*重置 mac len*/
...
perform_csum_check:
		if (!csum && !nskb->remcsum_offload) {
			nskb->csum = skb_checksum(nskb, doffset, /*计算csum值*/
						  nskb->len - doffset, 0);
			nskb->ip_summed = CHECKSUM_NONE;
			SKB_GSO_CB(nskb)->csum_start =
			    skb_headroom(nskb) + doffset;
