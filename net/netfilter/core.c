...
int nf_register_net_hook(struct net *net, const struct nf_hook_ops *reg)
{
	...
	hook_list = nf_find_hook_list(net, reg); /*ÔÚÕâ¸öÍøÂç¿Õ¼äÖÐÕÒµ½´æ´¢È«¾Ö¹³×Óº¯ÊýµÄÊý×é 
	                                                ²¢·µ»ØÏàÓ¦Ð­Òé×åºÍ¶ÔÓÚ¹³×ÓµãµÄ¹³×Óº¯ÊýµÄÁ´±íÍ·*/
	if (!hook_list) {
		kfree(entry);
		return -ENOENT;
	}
...
static int nf_register_hook_list(struct net *net)
{
	...
	list_for_each_entry(elem, &nf_hook_list, list) {/*±éÀú nf_hook_list  ÀïÃæºÃÏñÒ²Ã»¶«Î÷°¡ ²»ÖªµÀÎªÊ²Ã´ÒªÕâ¸ö
	                                                               nf_hook_list Ö»ÓÐÔÚ nf_register_hook µ÷ÓÃÁË°¡ */
...
static int __net_init netfilter_net_init(struct net *net)
{
	...
	for (i = 0; i < ARRAY_SIZE(net->nf.hooks); i++) {/*Ìí¼ÓÄ£¿éÊ± ÎªÃ¿¸öÍøÂç¿Õ¼ä¶¼³õÊ¼»¯ºÃ¹³×Ó´æ´¢Á´±í 
	                                                           Ô­À´ÊÇÓÐÒ»¸öÈ«¾ÖµÄnf_hooks ÏÖÔÚ¼ÓÈëÁËÍøÂç¿Õ¼ä*/
...
#endif
	ret = nf_register_hook_list(net);/*³õÊ¼»¯Ê±×¢²á¹³×Óº¯Êý ÔÝÇÒÕâÃ´Àí½â°É  */
	if (ret)
...
int __init netfilter_init(void)
{
	...
	ret = register_pernet_subsys(&netfilter_net_ops);/*½«Ò»¸öÄ£¿éÌí¼Óµ½Ã¿Ò»¸öÍøÂç¿Õ¼ä È»ºóÖ´ÐÐopsÖ¸ÏòµÄinitº¯Êý*/
	if (ret < 0)
		goto err;
